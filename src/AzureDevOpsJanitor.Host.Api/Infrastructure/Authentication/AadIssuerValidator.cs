...
namespace ResourceProvisioning.Broker.Host.Api.Infrastructure.Authentication
{
	...
	public class AadIssuerValidator
	{
		...
		public static AadIssuerValidator GetIssuerValidator(string aadAuthority)
		{
			...
			if (IssuerValidators.TryGetValue(aadAuthority, out AadIssuerValidator aadIssuerValidator))
			{
			...
			else
			{
				...
				var issuerMetadata = ConfigManager.GetConfigurationAsync().ConfigureAwait(false).GetAwaiter().GetResult();
				...
				var aliases = issuerMetadata.Metadata
					...
					.Distinct();
				IssuerValidators[authority] = new AadIssuerValidator(aliases);
				return IssuerValidators[authority];
			}
		}
